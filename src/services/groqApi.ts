...
**ข้อกำหนดทางเทคนิค:**
- \`entryPrice\`: หากราคาปัจจุบันเข้าได้เลย ให้ใส่เท่ากับราคาปัจจุบัน หากต้องรอโซน ให้ใส่ราคาโซนที่ต้องการ (ระบบจะคำนวณการรอให้อัตโนมัติ)
- \`support\` และ \`resistance\` ใน JSON ต้องเป็น **Array ของตัวเลข 2 ตัว** ที่แสดงถึง **ช่วงราคา (ขอบบนและขอบล่าง)** เสมอ เช่น [2400.50, 2390.00] **ห้ามใส่ตัวเลขเดียวกันซ้ำกัน** เพื่อแสดงพื้นที่ของโซน
    `;
...
{"currentPrice":${latestPrice.toFixed(2)},"trend":"BULLISH|BEARISH|SIDEWAYS","structure":"วิเคราะห์โครงสร้างตลาด H4/H1 และสภาวะปัจจุบันใน M30","keyLevels":{"support":[price_start,price_end],"resistance":[price_start,price_end]},"signal":{"type":"BUY|SELL|WAIT","entryPrice":num,"stopLoss":num,"takeProfit":num,"confidence":0-100,"reasoning":"อธิบายเหตุผล 4-6 ประโยค: 1) วิเคราะห์เทรนด์ H4 2) ระบุ Demand/Supply Zone สำคัญ 3) มองว่าราคาปัจจุบันเป็นการย่อตัวเข้าหาแนวรับหรือการเด้งหาแนวต้านหรือไม่ 4) ระบุเป้าหมายและการคุมความเสี่ยง"},"summary":"สรุปแผน Day Trade ประจำวัน"}`;
    }
}
    ...
    if (!response.ok) {
        ...
        if (response.status === 429) {
            try {
                ...
                if (message.includes('tokens per day (TPD)')) {
                    throw new Error('โควตาการใช้งานรายวันของ Groq (100,000 tokens) หมดแล้วครับ กรุณาลองใหม่ในวันถัดไป หรือสลับไปใช้ Gemini แทน');
                }
                if (message.includes('Please try again in')) {
                    const waitMatch = message.match(/try again in ([\w.]+)/);
                    const waitTime = waitMatch ? waitMatch[1] : 'สักครู่';
                    throw new Error(`Groq ติด Limit ชั่วคราว กรุณารออีกประมาณ ${waitTime} หรือสลับไปใช้ Gemini แทนครับ`);
                }
            } catch (e: any) {
                if (e.message.includes('Groq')) throw e;
            }
            throw new Error('Groq เรียกใช้งานบ่อยเกินไป (Rate Limit) กรุณารอสักครู่แล้วลองใหม่ครับ');
        }
